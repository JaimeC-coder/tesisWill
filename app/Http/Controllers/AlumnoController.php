    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($newRequest) {
            if(isset($newRequest['per_id_Alumno']) && $newRequest['per_id_Alumno'] != null && isset($newRequest['per_id_Apoderado']) && $newRequest['per_id_Apoderado'] != null ){
                ...
                $perUser = User::create([
                    'per_id' => $per_id_Apoderado->per_id,
                    'usu_email' => $newRequest['per_email_Apoderado'],
                    'usu_password' => bcrypt($newRequest['per_dni_Apoderado']),
                    'usu_estado' => 1,
                    'usu_rol' => 2
                ]);
                ...
                $perAlumnoUser = User::create([
                    ...
                    'usu_email' => $newRequest['per_email_Alumno'],
                    'usu_password' => bcrypt($newRequest['per_dni_Alumno']),
                    'usu_estado' => 1,
                    'usu_rol' => 3
                ]);
